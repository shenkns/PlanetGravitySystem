...
constexpr  float SWIMBOBSPEED = -80.0f;
...
#if WITH_EDITOR
void UGravityMovementComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
...
void UGravityMovementComponent::UpdateBasedRotation(const FQuat& DeltaRotation)
{
...
FVector UGravityMovementComponent::ComputeGroundMovementDeltaEx(const FVector& Delta, const FVector& DeltaPlaneNormal, const FHitResult& RampHit, const bool bHitFromLineTrace) const
{
	...
	if (!bHitFromLineTrace && IsWalkable(RampHit))
	{
		...
		if (!FGravityMath::Orthogonal(DeltaNormal, FloorNormal, ThresholdOrthogonalCosine))
		{
...
void UGravityMovementComponent::ComputeFloorDist(const FVector& CapsuleLocation, float LineDistance, float SweepDistance, FFindFloorResult& OutFloorResult, float SweepRadius, const FHitResult* DownwardSweepResult) const
{
	...
	if (DownwardSweepResult != NULL && DownwardSweepResult->IsValidBlockingHit())
	{
		...
		if (FGravityMath::Coincident((DownwardSweepResult->TraceEnd - DownwardSweepResult->TraceStart).GetSafeNormal(),
...
void UGravityMovementComponent::OnMovementUpdated(float DeltaSeconds, const FVector& OldLocation, const FVector& OldVelocity)
{
...
float UGravityMovementComponent::VisualizeMovement() const
{
	if (CharacterOwner == nullptr)
	{
		return 0.0f;
	...
	constexpr float OffsetPerElement = 10.0f;
...
void UGravityMovementComponent::OnClientCorrectionReceived(FNetworkPredictionData_Client_Character& ClientData, float TimeStamp, FVector NewLocation, FVector NewVelocity, UPrimitiveComponent* NewBase, FName NewBaseBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode)
{
#if !UE_BUILD_SHIPPING
	if (NinjaCharacterMovementCVars::NetShowCorrections != 0)
	{
...
void UGravityMovementComponent::OnComponentHit(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
...
void UGravityMovementComponent::UnwalkableHit(const FHitResult& Hit)
{
	OnUnwalkableHit(Hit);

	// Call owner delegate
	AGravityCharacter* Ninja = Cast<AGravityCharacter>(CharacterOwner);
	if (Ninja != nullptr)
	{
...
void UGravityMovementComponent::OnUnwalkableHit(const FHitResult& Hit)
{
}

bool UGravityMovementComponent::ShouldReplicateGravity() const
{
...
void UGravityMovementComponent::K2_SetFixedGravityDirection(const FVector& NewGravityDirection)
{
...
void UGravityMovementComponent::K2_SetPlaneGravityDirection(const FVector& NewGravityPlaneBase, const FVector& NewGravityPlaneNormal)
{
...
void UGravityMovementComponent::MulticastSetGravityScale_Implementation(float NewGravityScale)
{
	GravityScale = NewGravityScale;
}

void UGravityMovementComponent::SetAlignGravityToBase(bool bNewAlignGravityToBase)
{
...
void UGravityMovementComponent::MulticastEnableAlignGravityToBase_Implementation()
{
	bAlignGravityToBase = true;
}

void UGravityMovementComponent::MulticastDisableAlignGravityToBase_Implementation()
{
	bAlignGravityToBase = false;
}

void UGravityMovementComponent::UpdateGravity()
{
...
void UGravityMovementComponent::ReplicateGravityToClients()
{
...
FRotator UGravityMovementComponent::ConstrainComponentRotation(const FRotator& Rotation) const
{
	...
	return FGravityMath::MakeFromZQuat(CapsuleUp, Rotation.Quaternion(),
		ThresholdParallelCosine).Rotator();
}

void UGravityMovementComponent::SetAlignComponentToFloor(bool bNewAlignComponentToFloor)
{
...
void UGravityMovementComponent::MulticastEnableAlignComponentToFloor_Implementation()
{
	bAlignComponentToFloor = true;
}

void UGravityMovementComponent::MulticastDisableAlignComponentToFloor_Implementation()
{
	bAlignComponentToFloor = false;
}

void UGravityMovementComponent::SetAlignComponentToGravity(bool bNewAlignComponentToGravity)
{
...
void UGravityMovementComponent::MulticastEnableAlignComponentToGravity_Implementation()
{
	bAlignComponentToGravity = true;
}

void UGravityMovementComponent::MulticastDisableAlignComponentToGravity_Implementation()
{
...
FVector UGravityMovementComponent::GetComponentDesiredAxisZ() const
{
	...
	if (DesiredAxisZ.Z == 1.0f ||
		FGravityMath::Coincident(DesiredAxisZ, FVector::UpVector, ThresholdParallelCosine))
	{
...
bool UGravityMovementComponent::SetComponentAxisZ(const FVector& NewComponentAxisZ, bool bForceFindFloor)
{
	if (!HasValidData())
	{
		return false;
	...
	{
		if (bForceFindFloor && !bLandOnAnySurface)
		{
			bLandOnAnySurface = true;
			FindFloor(UpdatedComponent->GetComponentLocation(), CurrentFloor, false);
			bLandOnAnySurface = false;
		}
		else
...
bool UGravityMovementComponent::UpdateComponentRotation(const FVector& DesiredAxisZ, bool bRotateAroundCenter, bool bRotateVelocity)
{
	if (!HasValidData())
	{
		return false;
	...
	const FQuat PawnRotation = UpdatedComponent->GetComponentQuat();
	const FVector CurrentAxisZ = FGravityMath::GetAxisZ(PawnRotation);

	// Abort if angle between new and old capsule 'up' axes almost equals to 0 degrees
	if (FGravityMath::Coincident(DesiredAxisZ, CurrentAxisZ, ThresholdParallelCosine))
	{
		return false;
	}
	...
	const FQuat NewRotation = FGravityMath::MakeFromZQuat(DesiredAxisZ, PawnRotation, ThresholdParallelCosine);
...
void UGravityMovementComponent::SetThresholdParallelAngle(float NewThresholdParallelAngle)
{
